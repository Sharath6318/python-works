...
required_data = {nam_mdl.get("name") : nam_mdl.get("model") for nam_mdl in vehicles}

required_data_val = [cnt for cnt in required_data.values()]

count_val = {cou : required_data_val.count(cou) for cou in required_data_val}

max = max(count_val.values())

for k, v in count_val.items():

    if v ==  max:

        print(f"The {k} model has the maximum number of vehicles: {v}")